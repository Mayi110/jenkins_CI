...
# 生成log文件
log_file = os.path.join(os.getcwd(),'log/%s_api.log'%time.strftime('%Y%m%d%H%M%S'))
...
def runTest(testCaseFile):
    ...
    if not os.path.exists(testCaseFile):
        ...
        sys.exit()
    testCase = xlrd.open_workbook(testCaseFile,'wb+')
...
def interfaceTest(num,api_purpose,api_host,request_url,request_data,check_point,request_method,request_data_type,session):
    headers = {'Content-Type':'application/json; charset=UTF-8',
               ...
               'x-auth-token':'5098E8D9F71D693D9912E00652D7D6B0',
    ...
    headers_info = {'Content-Type': 'application/json','x-auth-token':'5098E8D9F71D693D9912E00652D7D6B0'}
    if session is not None:
        ...
        if request_data_type == 'Form':
            headers_info['Content-Type'] = 'Content-Type: multipart/form-data; boundary=---------------------------7d159c1302d0y0'


    conn = http.client.HTTPConnection(api_host)
    if request_method == 'POST':
        conn.request('POST',request_url,request_data,headers=headers_info)
        #r = requests.post('http://' + api_host+request_url, data=request_data, headers=headers)
    elif request_method == 'POST_FORM':

        #转换dist类型为byte类型
        params = urllib.parse.urlencode(eval(request_data.decode()))

        conn.request('POST',request_url,params, headers=headers)
    ...
    resp = response.read()
    print(u'预期结果：',check_point)
    print(u'请求URL：',request_url)
    if status == 200:
...
#接口返回与预期不匹调用test_wx发送错误通知
def main():

    errorTest = runTest('TestCase/TestCase_coupon.xlsx')
    if len(errorTest) > 0:
       send_msg(errorTest)
       print("测试结果：Failed")
    else:
        print("测试结果：Succesfully")
